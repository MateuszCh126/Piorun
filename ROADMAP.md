...
| # | Zadanie | Pliki | Rozmiar | Ryzyko |
|---|---------|-------|---------|--------|
| 3.1 | ✅ ZREALIZOWANE (2026-07-07): dyspozytor `piorun.py [chat\|doctor\|digest\|autonomy ...]`; bez argumentów = chat (`-c` i .bat zachowane); `autonomy` deleguje do `scripts/autonomy_ops.py`; `digest` aktywny po wdrożeniu 2.2 | `piorun.py` | M | — |
| 3.2 | ✅ ZREALIZOWANE (2026-07-07): `core/doctor.py` — LLM, model Whisper, ddgs, embeddingi, credentials, wolne miejsce, loopback, bazy; zweryfikowany na żywo (8/9, LLM offline) | `core/doctor.py` | M | — |
